...
pub fn get_dest_hashmap<'a>() -> HashMap<&'a str, u16>{
  let dest_mnemonic = ["","M","D","MD","A","AM","AD","AMD"];
  let dest_hashmap: HashMap<_,_> = dest_mnemonic.into_iter().zip(0..=7).collect();
  dest_hashmap
}
pub fn get_comp_hashmap<'a>() -> HashMap<&'a str, u16>{
  let comp_mnemonic = ["0","1","-1","D","A","!D","!A","-D","-A","D+1",
                                      "A+1","D-1","A-1","D+A","D-A","A-D","D&A","D|A",
                                      "M","!M","-M","M+1","M-1","D+M","D-M","M-D","D&M","D|M"];
  let comp_num = [42, 63, 58, 12, 48, 13, 49, 15,
...
pub fn get_jump_hashmap<'a>() -> HashMap<&'a str, u16>{
  let jump_mnemonic = ["","JGT","JEQ","JGE","JLT","JNE","JLE","JMP"];
...
pub fn get_symbol_hashmap<'a>(symbolr_str: &'a Vec<String>) -> HashMap<&'a str, u16>{
  let symbol_str = ["SP","LCL","ARG","THIS","THAT","SCREEN","KBD"];

  let symbol_num  = [0, 1, 2, 3, 4, 16384, 24576];
